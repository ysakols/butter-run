...
final class DeepLinkRouter {
    /// The pending destination to navigate to. Views consume and nil-out after handling.
    ...
    static func parse(_ url: URL) -> DeepLinkDestination? {
        guard url.scheme == "butterrun" else { return nil }
        guard url.host() == "run" else { return nil }

        // Path: /UUID  (url.host == "run", pathComponents == ["/", "UUID"])
